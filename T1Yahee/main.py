...
def createWorkbook(T1, verzollung, Sendungsnr, Schiff, BL,BLDatum, Rechnungsnr, RechnungsDatum, Containernr, Incoterm,
    ...
    T1Sheet.write('B4', RechnungsDatum)
...
def createFile(Sdus,Schiff,BL,BLDATUM,rechnungsnr,rechnungsdatum,Containernr,Incoterm,Transportpreis,Inlandspreis):
    ...
    writer = pd.ExcelWriter(Sendungsnr + ".xlsx", engine='xlsxwriter')
    T1, verzollung = createT1('test.xlsx')
    createWorkbook(T1, verzollung, Sendungsnr, Schiff1, BL1, BLDatum ,Rechnungsnr,Rechnungsdatum,Containernr1, Incoterm1, Transportpreis1, Inlandpreis1,writer)
    print('done')
    return

canvas1 = tk.Canvas(hauptFenster, width=400, height=100)
...
SdusButton = tk.Button(text='SDUS', command=lambda: createFile(Sdus,Schiff,BL,BLDATUM,rechnungsnr,rechnungsdatum,Containernr,Incoterm,Transportpreis,Inlandspreis))
SdusButton.pack()
